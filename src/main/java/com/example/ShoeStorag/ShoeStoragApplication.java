...
public class ShoeStoragApplication {
	...
	public CommandLineRunner demo(ShoeRepository repository, TypeRepository trepository, UserRepository urepository) {
		return (args) -> {
			...
			repository.save(new Shoe("Nike", "Air Force", "Men", "45", "119 €", trepository.findByTypename("Sneaker").get(0)/*, user1*/));
			repository.save(new Shoe("Adidas", "Adilette Aqua Swim", "Women", "38", "20 €", trepository.findByTypename("Slipper").get(0)/*, user1*/));
			repository.save(new Shoe("Asics", "Jolt 3", "Child", "33", "32 €", trepository.findByTypename("Running").get(0)/*, user1*/));
